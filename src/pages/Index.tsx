...
const Index = () => {
  ...
  useEffect(() => {
    ...
    if (searchTerm.trim() !== "") {
      filtered = filtered.filter(term => term.term.toLowerCase().includes(searchTerm.toLowerCase()) || term.definition.toLowerCase().includes(searchTerm.toLowerCase()) || term.tags.some(tag => tag.toLowerCase().includes(searchTerm.toLowerCase())));
    }
  ...
  return <div className="min-h-screen bg-background">
      ...
      <header className="text-center py-16 px-4 border-b border-border/10">
        ...
        <div className="max-w-2xl mx-auto relative">{/* matches description width */}
          ...
          <Input type="text" placeholder="Search crypto terms" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-12 pr-12 h-14 text-lg border border-border/20 bg-card/30 backdrop-blur-sm hover:bg-card/50 transition-all duration-300 focus:ring-2 focus:ring-primary/50 rounded-xl" />
          ...
          <div className="absolute right-3 top-1/2 transform -translate-y-1/2 flex gap-1">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent className="w-48 bg-background border border-border/20 shadow-lg z-50">
                ...
                <DropdownMenuItem onClick={() => setSortBy("default")} className={sortBy === "default" ? "bg-muted/50" : ""}>
                  Default
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => setSortBy("a-z")} className={sortBy === "a-z" ? "bg-muted/50" : ""}>
                  A-Z
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => setSortBy("z-a")} className={sortBy === "z-a" ? "bg-muted/50" : ""}>
                  Z-A
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => setSortBy("difficulty-asc")} className={sortBy === "difficulty-asc" ? "bg-muted/50" : ""}>
                  Beginner → Advanced
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => setSortBy("difficulty-desc")} className={sortBy === "difficulty-desc" ? "bg-muted/50" : ""}>
                  Advanced → Beginner
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => setSortBy("category")} className={sortBy === "category" ? "bg-muted/50" : ""}>
                  Category
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-muted-foreground hover:text-foreground hover:bg-transparent">
                  ...
                  {(selectedCategories.length > 0 || selectedDifficulties.length > 0) && <div className="absolute -top-1 -right-1 h-3 w-3 bg-primary rounded-full"></div>}
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent className="w-64 bg-background border border-border/20 shadow-lg z-50">
                <DropdownMenuLabel>Categories</DropdownMenuLabel>
                {categories.map(category => <DropdownMenuCheckboxItem key={category} checked={selectedCategories.includes(category)} onCheckedChange={() => handleCategoryToggle(category)} className="hover:bg-muted/50">
                    {category}
                ...
                {difficulties.map(difficulty => <DropdownMenuCheckboxItem key={difficulty} checked={selectedDifficulties.includes(difficulty)} onCheckedChange={() => handleDifficultyToggle(difficulty)} className="hover:bg-muted/50">
                    {difficulty}
                  </DropdownMenuCheckboxItem>)}
      ...
      <main className="container mx-auto px-4 py-12">
        <Accordion type="multiple" className="max-w-4xl mx-auto space-y-4">
          {filteredTerms.map((term, index) => <AccordionItem key={term.id} value={term.id} className="border-none">
              <Card className="hover:shadow-lg transition-all duration-300 border border-border/20 bg-card/30 backdrop-blur-sm hover:bg-card/50 rounded-xl" style={{
            animationDelay: `${index * 0.02}s`
                  ...
                  <CardHeader className="pb-3 w-full">
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <div className="flex gap-2 ml-4">
                        ...
                        {term.difficulty && <div className="flex items-center gap-0.5">
                ...
                <AccordionContent className="pb-0">
                  <CardContent className="pt-0 space-y-6">
                    ...
                    {/* Use Cases */}
                    {term.examples && term.examples.length > 0 && <div>
                        <h4 className="text-sm font-semibold text-foreground mb-2 flex items-center gap-2">
                          <BookOpen className="h-4 w-4 text-primary" />
                          Examples & Use Cases
                        </h4>
                        <ul className="space-y-2">
                          {term.examples.map((example, idx) => <li key={idx} className="text-sm text-muted-foreground flex items-start gap-2">
                    ...
                    {/* Related Terms/Tags */}
                    {term.tags && term.tags.length > 0 && <div>
                        <h4 className="text-sm font-semibold text-foreground mb-3 flex items-center gap-2">
                          <Globe className="h-4 w-4 text-primary" />
                          Related Terms
                        </h4>
                        <div className="flex flex-wrap gap-2">
                          {term.tags.map(tag => <Badge key={tag} variant="outline" className="text-xs px-3 py-1.5 bg-muted/30 text-muted-foreground border-muted hover:bg-primary/10 hover:text-primary hover:border-primary/20 transition-colors cursor-pointer">
                              {tag}
                            </Badge>)}
                        </div>
                      </div>}
        ...
        {filteredTerms.length === 0 && searchTerm && <div className="text-center py-16">
